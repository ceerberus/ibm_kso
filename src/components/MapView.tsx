...
export default function MapView({ activities, onActivityClick }: MapViewProps) {
  ...
  useEffect(() => {
    ...
    activities.forEach((activity) => {
      ...
      const capacityPercent = (activity.spotsTaken / activity.totalSpots) * 100;
      ...
      el.innerHTML = `
        ...
        ">
          ...
          <span style="font-weight: 600; color: ${spotColor};">${activity.spotsTaken}/${activity.totalSpots}</span>
        </div>
      `;
      ...
      const popup = new maplibregl.Popup({
        ...
        ">
          ...
          <div style="
            ...
            color: #717171;
            margin-bottom: 6px;
          ">${new Date(activity.date).toLocaleDateString('en-CH', { day: 'numeric', month: 'short' })} · ${activity.time || 'TBD'}</div>
          <div style="
            font-size: 13px;
            color: #222222;
            font-weight: 500;
          ">${activity.spotsTaken} of ${activity.totalSpots} spots filled</div>
        </div>
      `);
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-white/10 h-[400px] md:h-[500px] relative">
